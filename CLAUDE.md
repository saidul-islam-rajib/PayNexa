...
- Controllers derive from `ApiControllerBase`: no route strings for the resource (`api/v{version}/[controller]`, version from the `Controllers.V<n>` namespace, kebab-case URLs) and no `[ProducesResponseType]` (added by convention from the HTTP method). Action routes come from `ApiRoutes` (building blocks) and `<Service>.API/Routing` constants, never inline strings. Actions are expression-bodied one-liners returning `Task<ActionResult<T>>` via `QueryAsync` / `CommandAsync` / `CreateAsync` (no injected `ISender`). Large controllers are split into partial files by concern (`CustomersController.Kyc.cs`). Query-string models live in the API layer (`Requests`), paged ones derive from `PagedRequest`.
